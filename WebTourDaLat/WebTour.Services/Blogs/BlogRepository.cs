...
public class BlogRepository : IBlogRepository
{
    ...
    {
        return await _context.Set<Posts>()
  ...
  CancellationToken cancellationToken = default)
    {
        var post = await _context.Set<Posts>().FindAsync(postId);
        if (post is null) return false;
        _context.Set<Posts>().Remove(post);
        var rowsCount = await _context.SaveChangesAsync(cancellationToken);
        return rowsCount > 0;
    }
    ...
    public async Task<Posts> GetPostByIdAsync(
        int postId, bool includeDetails = false,
        CancellationToken cancellationToken = default)
    {
        if (!includeDetails)
        {
            return await _context.Set<Posts>().FindAsync(postId);
        }
    ...
    public async Task<Posts> GetPostBySlugAsync(
    ...
    public async Task<Posts> FindPostByIDAsync(
        int id,
        CancellationToken cancellationToken = default)
    {
        return await _context.Set<Posts>()
    ...
    public BlogDbContext Get_context()
    {
        return _context;
    }
    ...
    public async Task AddOrUpdatePostAsync(
       Posts postName, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<Posts> CreateOrUpdatePostAsync(
            Posts post, IEnumerable<string> tags,
            CancellationToken cancellationToken = default)
    {
        if (post.Id > 0)
        {
    ...
    public async Task<IList<Posts>> GetRandomPostsAsync(
        int n,
        CancellationToken cancellationToken = default)
    {
        return await _context.Set<Posts>()
    ...
    public async Task<IList<Posts>> GetAllPostsByPostQuery(
        PostsQuery pquery, CancellationToken cancellationToken = default)
    {
        return await _context.Set<Posts>()
    ...
    public async Task<IList<Posts>> FindAllPostsWithPostQueryAsync(PostsQuery pq, CancellationToken cancellationToken = default)
    {
        return await _context.Set<Posts>()
                .Include(c => c.Category)
                .Include(t => t.Tags)
    ...
    public async Task<int> CountPostsWithPostQueryAsync(PostsQuery pq,
             CancellationToken cancellationToken = default)
    {
        return await _context.Set<Posts>()
            .Include(c => c.Category)
    ...
    public async Task<IPagedList<Posts>> GetPagedsPostAsync(
        PostsQuery pq, IPagingParams pagingParams,
        CancellationToken cancellationToken = default)
    {
    ...
    public IQueryable<Posts> FilterPost(PostsQuery pq)
    {
   ...
   PostsQuery condition,
   IPagingParams pagingParams,
   Func<IQueryable<Posts>, IQueryable<T>> mapper)
    {
    ...
    public async Task<IPagedList<Posts>> GetPagedPostAsync(
            PostsQuery pq,
            int pageNumber = 1,
            int pageSize = 10,
            CancellationToken cancellationToken = default)
    ...
    public async Task<Posts> GetCachedPostByIdAsync(int postId)
    {
        return await _memoryCache.GetOrCreateAsync(
            $"post.by-id.{postId}",
            async (entry) =>
